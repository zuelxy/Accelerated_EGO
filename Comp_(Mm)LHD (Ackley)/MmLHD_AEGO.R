MmLHD_AEGO <- function(fun, model, npoints, optimum, eps, lower, upper, m,
                       minimization = TRUE, control = NULL)
{
  ...
  while(TRUE){ 
    ...
    Dii = lhsDesign(m,length(lower))$design
    LHD = maximinSA_LHS(Dii)$design   ####### [0,1]^d
    LHD_domain <- affine(LHD, lower, upper)
