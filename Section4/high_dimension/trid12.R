...
U <- read.table("K:/Accelerated_EGO_code/section_4/high_dimension/UD12_120.txt", header = T)
...
QMC <- sobol(1200, dim = p, scrambling = 3)
QMC <- affine(QMC, lower, upper)
  
begin_AEGO <- proc.time()
result <- AEGO_nsteps(QMC = QMC, fun = trid, model = fitted_model, npoints = q, 
                        nsteps = nsteps, lower = lower, upper = upper,
                        control = list(max.generations = 100, 
                                       wait.generations = 10, BFGSburnin = 10))
...
############################## Constant Liar ##########################
begin_CL <- proc.time()
result_CL <- qEGO_nsteps(fun = levy, model = fitted_model, npoints = q, nsteps = nsteps,
...
begin_EGO <- proc.time()
result_EGO <- EGO_nsteps(model = fitted_model, fun = levy, nsteps = nsteps_e,
