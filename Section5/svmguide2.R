...
######### data prepare #######
svmguide2 <- read_excel("K:/Accelerated_EGO_code/section_5/svm_data/svmguide2.xlsx")
svmguide2$class <- factor(svmguide2$class)  # transform 'class' to factor type
str(svmguide2)   # check the type of data

set.seed(87)
index <- sample(1:nrow(svmguide2), size = 0.75*nrow(svmguide2))
train <- as.data.frame(svmguide2[index,])    
test <- as.data.frame(svmguide2[-index,])    # divide the train and test data 
...
for (k in 1:10) {
  begin_g <- proc.time()
  obj <- tune.svm(x, y, gamma = 2^(-15:5), cost = 2^(-5:15), 
