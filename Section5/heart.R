...
######### data prepare #######
heart <- read_excel("K:/Accelerated_EGO_code/section_5/svm_data/heart.xlsx")
heart$class <- factor(heart$class)  # transform 'class' to factor type
str(heart)   # check the type of data

set.seed(999)
index <- sample(1:nrow(heart), size = 0.75*nrow(heart))
train <- as.data.frame(heart[index,])    
test <- as.data.frame(heart[-index,])    # divide the train and test data 
...
for (k in 1:10) {
  begin_g <- proc.time()
  obj <- tune.svm(x, y, gamma = 2^(-20:0), cost = 2^(0:20), 
