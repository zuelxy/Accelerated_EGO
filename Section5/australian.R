...
######### data prepare #######
australian <- read_excel("K:/Accelerated_EGO_code/section_5/svm_data/australian.xlsx")
australian$class <- factor(australian$class)  # transform 'class' to factor type
str(australian)   # check the type of data

set.seed(18)
index <- sample(1:nrow(australian), size = (2/3)*nrow(australian))
train <- as.data.frame(australian[index,])    
test <- as.data.frame(australian[-index,])    # divide the train and test data 
...
for (k in 1:10) {
begin_g <- proc.time()
obj <- tune.svm(x, y, gamma = 2^(-20:0), cost = 2^(-5:15))
...
fitted_model <- km(design = X, response = accuracy_X, optim.method = "gen",
                   covtype = "gauss", nugget=1e-3*var(accuracy_X), 
...
################################# constant liar ################################
begin_CL <- proc.time()
result_CL <- qEGO_nsteps(fun = svm_accuracy, svmx = x, svmy = y, model = fitted_model, npoints = q,
...
nsteps <- 21
begin_EGO <- proc.time()
result_EGO <- EGO_nsteps(model = fitted_model, fun = svm_accuracy, svmx = x, svmy =y, lower = lower, 
                           upper = upper, nsteps = nsteps, minimization = FALSE,
                           control = list(pop.size = 50, max.generations = 50, 
                                       wait.generations = 5, BFGSburnin = 5))
