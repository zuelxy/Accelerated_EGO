...
######### data prepare #######
german_numer <- read_excel("K:/Accelerated_EGO_code/section_5/svm_data/german_numer.xlsx")
german_numer$class <- factor(german_numer$class)  # transform 'class' to factor type
str(german_numer)   # check the type of data

set.seed(97)
index <- sample(1:nrow(german_numer), size = 0.75*nrow(german_numer))
train <- as.data.frame(german_numer[index,])    
test <- as.data.frame(german_numer[-index,])    # divide the train and test data 
...
for (k in 9:10) {
begin_g <- proc.time()
obj <- tune.svm(x, y, gamma = 2^(-25:-5), cost = 2^(0:20),
